...
-- where coalesce(canonical_id, '') in (select ${canonical_id_name} from ${dest_db}.${unif_src_tbl}_inc_matched_id)
-- ;
...
as
select * from cdp_unification_${unif_name}.${canonical_id_name}_lookup_full
where coalesce(canonical_id, '') not in (select ${canonical_id_name} from ${dest_db}.${unif_src_tbl}_inc_matched_id)
;
