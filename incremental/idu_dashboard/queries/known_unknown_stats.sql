WITH KNOW AS (
  SELECT COUNT(DISTINCT ${id_lookup_main_id}) as cnt_known
...
ADDRESSABLE AS (
    SELECT COUNT(DISTINCT ${id_lookup_main_id}) as cnt_addressable
FROM ${source_db}.${canonical_id_col}_lookup
...
EMAIL AS (
    SELECT COUNT(DISTINCT ${id_lookup_main_id}) as cnt_email
FROM ${source_db}.${canonical_id_col}_lookup
WHERE id_key_type IN (SELECT DISTINCT key_type FROM idu_column_mapping WHERE id_context = 'email' )
),

PHONE AS (
    SELECT COUNT(DISTINCT ${id_lookup_main_id}) as cnt_phone
FROM ${source_db}.${canonical_id_col}_lookup
WHERE id_key_type IN (SELECT DISTINCT key_type FROM idu_column_mapping WHERE id_context = 'phone' )
),

EVERYONE AS (
SELECT COUNT(DISTINCT ${id_lookup_main_id}) as total_profiles FROM ${source_db}.${canonical_id_col}_lookup
)
