...
column_config tc
left join
(select distinct key_type, key_name from ${globals.canonical_id}_keys) k
...
'when cast(nullif(p.' || table_field || ', '''') as varchar) is not null then cast(p.' || table_field || ' as varchar)'
else
'when cast(nullif(p.' || table_field || ', '''') as string) is not null then cast(p.' || table_field || ' as string)'
end as id_case_sub_query,
case when engine = 'presto' then
'when cast(nullif(p.' || table_field || ', '''') as varchar) is not null then ' || coalesce(cast(key_type as varchar),'no key')
else
...
join_conditions as (select
  ...
  bucket_cols,
  case when engine = 'presto' then
  'left join cdp_unification_${unif_name}.${globals.canonical_id}_lookup_full k0' || chr(10) || '  on k0.id = case ' || array_join(array_agg(id_case_sub_query),chr(10)) || chr(10) || 'else null end'
  else
  'left join cdp_unification_${unif_name}.${globals.canonical_id}_lookup_full k0' || chr(10) || '  on k0.id = case ' || array_join(array_agg(id_case_sub_query),chr(10)) || chr(10) || 'else ''null'' end' 
  end as id_case_sub_query,
  case when engine = 'presto' then
...
FROM (
    ...
    concat('nullif(p.', column_name, ',', '''''' ,') as ', column_name)  column_name
	  FROM information_schema.COLUMNS
