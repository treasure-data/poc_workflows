WITH T0 AS (
  SELECT ${id_lookup_main_id}, id_key_type, id FROM ${id_lookup_table}
  WHERE ${id_lookup_main_id} IN (SELECT over_merged_id FROM idu_qa_over_merged_canonical_ids)
),
T1 AS (
SELECT ${id_lookup_main_id}, id_key_type, APPROX_DISTINCT(id) AS id_cnt 
FROM  T0
GROUP BY 1, 2
ORDER by 3 desc
),
